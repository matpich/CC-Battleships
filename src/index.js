...
//######################
// Strzały a contrario do rozmieszczania rozmieszczenia
const placeGuess = player => {
  let Guess = 0;

  let battlefield;

  if (player.playerNo === 1) {
    battlefield = document.querySelector('#second-battlefield-prev');
  } else if (player.playerNo === 2) {
    battlefield = document.querySelector('#first-battlefield-prev');
  }


    //funkcja przekazywana do eventListenera
    const position = e => {
      if (Guess != 8 && e.toElement.className === 'field-prev') {
        const cords = {
          x: e.toElement.id[0],
          y: e.toElement.id[2]
        };
        player.ships[Guess].setGuessPosition(cords, player.ownFleetBoard);
      }
    };
  //wydarzenie odpalające się przy nacisnięciu klawisza myszy
  battlefield.addEventListener('mousedown', position);
  ...
  battlefield.addEventListener('mouseup', () => {
    ...
    if (Guess == 8) {
      ...
      const anyNotPositioned = player.ships[Guess].anyPositioned();
      if (anyNotPositioned) {
        return;
      } else {
        Guess;
...
placeGuess(playerOne);
placeGuess(playerTwo);
